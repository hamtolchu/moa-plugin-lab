...
function registerTools(server) {
    // ── Step 1: scaffold ─────────────────────────────────────────────────────
    server.tool("mock_ui_scaffold", "보일러플레이트를 클론하고 디자인 리소스를 복사해 mock 디렉토리를 초기화합니다. " +
    ...
    // ── Deploy (fresh) ───────────────────────────────────────────────────────
    server.tool("mock_ui_deploy", "로컬 mock을 Vercel에 프로덕션 배포하고 공개 URL을 반환합니다. VERCEL_TOKEN이 필요합니다. " +
    ...
    server.tool("mock_ui_redeploy", "dev 서버를 정지하고 현재 상태를 Vercel에 재배포합니다. {slug, prodUrl}를 반환합니다.", slugSchema, async (args) => {
        const { slug } = args;
        try {
    ...
    // ── Stop dev server ──────────────────────────────────────────────────────
    server.tool("mock_ui_stop", "로컬 dev 서버를 정지합니다. 이미 꺼져 있어도 오류 없이 완료됩니다. {slug, stopped: true}를 반환합니다.", slugSchema, async (args) => {
        const { slug } = args;
        stopDevServer(archivePath(slug));
        return { content: [{ type: "text", text: JSON.stringify({ slug, stopped: true }) }] };
    });
    // ── List all mocks ───────────────────────────────────────────────────────
    server.tool("mock_ui_list", "로컬에 저장된 모든 mock 목록을 최신순으로 반환합니다.", {}, async () => {
        const mocks = listMocks();
        return { content: [{ type: "text", text: JSON.stringify(mocks, null, 2) }] };
    });
    // ── Status of one mock ───────────────────────────────────────────────────
    server.tool("mock_ui_status", "특정 mock의 dev 서버 실행 상태, 로컬 URL, 배포 URL을 반환합니다.", slugSchema, async (args) => {
        const { slug } = args;
        try {
