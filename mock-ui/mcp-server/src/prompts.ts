// @ts-nocheck — McpServer.registerPrompt() overloads cause TS2589 in SDK v1.x
...
export function registerPrompts(server: McpServer): void {
  server.registerPrompt(
    "mock-ui",
    {
      title: "Mock UI 생성",
      description:
        "자연어 요구사항으로 단일 화면 Mock UI를 로컬에 생성하고 미리보기 URL을 반환합니다. " +
        "Vercel 배포는 사용자가 검토 후 별도로 요청해야 실행됩니다.",
      argsSchema: {
        description: z.string().describe("화면 요구사항을 자연어로 설명"),
      },
    },
    async ({ description }) => ({
      messages: [
